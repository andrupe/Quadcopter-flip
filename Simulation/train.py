...
# ======================================================================================
TOTAL_TIMESTEPS: int = 55_000_000    # Total training steps (permits 5M pre-anneal + 200 iters anneal)
...
# Phase 1 (0 -> 5M steps): Linear decay from LR_START (3e-4) to LR_MID (3e-5) [matches original 5M run exactly]
# Phase 2 (5M -> 5M + anneal_steps): Slower linear decay from LR_MID (3e-5) to LR_FLOOR (1e-5) [11x slower slope]
# Phase 3 (post-anneal -> end): Stable floor at LR_FLOOR (1e-5) to maintain policy plasticity under disturbances
LEARNING_RATE: float = 3e-4          # Initial PPO learning rate (Phase 1 start)
LR_START: float = 3e-4               # Initial PPO learning rate (Phase 1 start)
LR_MID: float = 3e-5                 # Target learning rate at 5,000,000 steps (Phase 1 end / Phase 2 start)
LR_FLOOR: float = 1e-5               # Fine-tuning floor learning rate
LR_WARMUP_STEPS: int = 5_000_000     # Phase 1 warmup duration (steps) before secondary LR decay begins
LR_FINAL_STABLE: bool = True         # Keep stable at LR_FLOOR during final post-training phase
...
LOAD_PREVIOUS_MODEL: bool = True     # Set to True to resume training from previous model, or False to start fresh
PREVIOUS_MODEL_PATH: Optional[str] = "quad_flip_model.zip"
...
DR_END_STEPS: int = 12_000_000        # Reach full DR (dr_level=1.0) by this step
...
class PiecewiseLinearSchedule:
    ...
    def __call__(self, progress_remaining: float) -> float:
        ...
        if current_step <= self.warmup_steps:
            # Phase 1: 3e-4 -> 3e-5 (slope = -5.4e-11 / step)
            ...
            # Phase 2: 3e-5 -> 1e-5 (slope = -4.88e-12 / step, 11x slower)
            p = (current_step - self.warmup_steps) / max(1, self.anneal_steps)
            return float(self.lr_mid - p * (self.lr_mid - self.lr_floor))
        else:
            ...
            if self.final_stable:
                ...
                return float(max(1e-6, self.lr_floor - p * self.lr_floor))
...
def train(
    ...
    anneal_steps = 4_096_000  # Fixed anneal duration for LR schedule Phase 2
    ...
    print(f"  LR Schedule  : Phase 1 (0 -> {LR_WARMUP_STEPS:,}): {learning_rate:.1e} -> {lr_mid:.1e} (Identical to 5M run)")
    print(f"                 Phase 2 ({LR_WARMUP_STEPS:,} -> {LR_WARMUP_STEPS + anneal_steps:,}): {lr_mid:.1e} -> {lr_floor:.1e} (11x slower decay)")
    print(f"                 Phase 3 (> {LR_WARMUP_STEPS + anneal_steps:,}): {'Stable at ' + f'{lr_floor:.1e}' if lr_final_stable else 'Linear decay to 0'}")
    ...
    # Initialize 3-phase piecewise schedule
