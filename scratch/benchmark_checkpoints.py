...
def run_benchmark():
    ...
    for ckpt_label, ckpt_base in CHECKPOINTS:
        ...
        if not os.path.isfile(zip_path) or not os.path.isfile(norm_path):
        ...
        for dr in DR_LEVELS:
            ...
            for ep_idx in range(EPISODES_PER_DR):
                ...
                for step in range(MAX_STEPS):
                    obs_norm = vec_norm.normalize_obs(obs)[:51]
            ...
            summary_rows.append({
                ...
                "arena_breach_count": term_reasons.get("arena_breach", 0),
            })
